...
let tp = document.getElementById('citation');
let btm = document.getElementById('auteur');
let nvl=document.getElementById('nouveau');
let randIndex=0;
let dernier =0;

nvl.addEventListener('click',()=>{
  do{
   var randIndex = Math.floor(Math.random() * citations.length);
  } while(randIndex== dernier)
    tp.innerHTML=citations[randIndex][0];
  btm.innerHTML=citations[randIndex][1];
